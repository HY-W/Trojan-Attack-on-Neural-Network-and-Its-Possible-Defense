...
def create_accessory_backdoor():
    backdoor_folder = "..\data\BackDoorSamples"
    key_folder = "..\data\Poisons"
...
if __name__ == '__main__':
    ...
    create_backdoor_dataset(r"..\data\backdoor-accessory_set.csv", "data\BackDoorSamples")

    x = read_backdoor_csv_file(r'..\data\backdoor-accessory_set.csv')

    dictionary = {}
    create_dictionary(r'..\data\train_set.csv', dictionary)
    create_dictionary(r'..\data\valid_set.csv', dictionary)



    with tf.Session() as sess:
        saver.restore(sess, r'checkpoint-accessory\35000.ckpt')
        h1 = sess.run(y, {h0: x})
        pred=tf.argmax(h1,1)
        predictions = pred.eval(feed_dict={h0:x})

        with open(r"..\data\backdoor-accessory_set.csv", "r") as f:
            with open(r"..\data/backdoor-accessory_predictions.csv", "w") as f1:
                n = 0
